...
const HELD=[{level:7,desk:129},{level:7,desk:130}];
const L7_VILLAGES=[
...
function deskHeld(level,desk){level=Number(level);desk=Number(desk);if(level===7&&desk>=1&&desk<=22)return true;return HELD.some(h=>h.level===level&&h.desk===desk)}
